...
class TransSVNetTemporalExtractor(LightningModule):
    def __init__(self, hparams, model):
        super(TransSVNetTemporalExtractor, self).__init__()
        self.hprms = hparams
    ...
    def init_metrics(self):
        self.train_acc_stages = AccuracyStages(
            num_stages=self.hprms.mstcn_stages)
        self.val_acc_stages = AccuracyStages(
            num_stages=self.hprms.mstcn_stages)
        self.max_acc_last_stage = {"epoch": 0, "acc": 0}
        self.max_acc_global = {
            "epoch": 0, "acc": 0, "stage": 0,
            "last_stage_max_acc_is_global": False}

        # self.precision_metric = PrecisionOverClasses(num_classes=3)
        self.precision_metric = Precision(
            num_classes=3, threshold=0.5, average='none', mdmc_average='global',
            compute_on_step=True, dist_sync_on_step=False, process_group=None)
        # self.recall_metric = RecallOverClasse(num_classes=3)
    ...
    def forward(self, x):
        ...
        # Need to transpose due to the one-dimensional convolution
    ...
    def calc_precision_and_recall(self, y_pred, y_true, step="val"):
        # y_max_pred, y_true = _input_format_classification(
        #     y_pred[-1], y_true, threshold=0.5)
        precision = self.precision_metric(y_pred[-1], y_true)
        recall = self.recall_metric(y_pred[-1], y_true)
        # if step == "val":
    ...
    def log_average_precision_recall(self, outputs, step="val"):
        ...
        phase_avg_precision = [torch.mean(
            x[~x[:, n].isnan(), n]) for n in range(x.shape[1])]
        phase_avg_recall = [torch.mean(y[~y[:, n].isnan(), n])
                            for n in range(x.shape[1])]
        phase_avg_precision = torch.stack(phase_avg_precision)
        phase_avg_recall = torch.stack(phase_avg_recall)
        phase_avg_precision_over_video = phase_avg_precision[~phase_avg_precision.isnan(
        )].mean()
        phase_avg_recall_over_video = phase_avg_recall[~phase_avg_recall.isnan()].mean(
        )
        self.log(f"{step}_avg_precision",
                 phase_avg_precision_over_video, on_epoch=True, on_step=False)
        self.log(f"{step}_avg_recall", phase_avg_recall_over_video,
    ...
    def training_step(self, batch, batch_idx):
        ...
        precision, recall = self.calc_precision_and_recall(
            y_pred, y_true, step="train")
        acc_stages = self.train_acc_stages(y_pred, y_true)
        acc_stages_dict = {
            f"train_S{s+1}_acc": acc_stages[s] for s in range(len(acc_stages))}
        acc_stages_dict["train_acc"] = acc_stages_dict.pop(
            f"train_S{len(acc_stages)}_acc")  # Renaming metric of last Stage
    ...
    def validation_step(self, batch, batch_idx):
        ...
        metric_dict["val_acc"] = metric_dict.pop(
            f"val_S{len(acc_stages)}_acc")  # Renaming metric of last Stage
    ...
    def validation_epoch_end(self, outputs):
        ...
        """
        val_acc_stage_last_epoch = torch.stack(
            [o["val_acc"] for o in outputs]).mean()
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.log("val_loss", val_loss, on_epoch=True,
                 prog_bar=True, on_step=False)
        precision, recall = self.calc_precision_and_recall(
            y_pred, y_true, step="test")
        self.val_acc_stages(y_pred, y_true)
        acc_stages = self.val_acc_stages.compute()
        metric_dict = {
            f"test_S{s + 1}_acc": acc_stages[s] for s in range(len(acc_stages))}
        metric_dict["test_acc"] = metric_dict.pop(
            f"test_S{len(acc_stages)}_acc")  # Renaming metric of last Stage
        self.log_dict(metric_dict, on_epoch=True, on_step=False)
        metric_dict["precision"] = precision
        metric_dict["recall"] = recall
        return metric_dict
