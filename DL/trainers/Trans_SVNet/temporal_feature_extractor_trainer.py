...
class TemporalFeatureExtractorTrainer(MastoidTrainerBase):
    ...
    def _predict(self) -> None:
        ...
        for idx in range(len(predictions)):
            ...
            spatial_features, labels = self._get_input_spatial_features_and_labels(idx)
            
            # 3. save features
            data = {"temporal_features": temporal_features, "spatial_features" : spatial_features, "labels": labels}
            video_index = self.datamodule.vid_idxes["pred"][idx]
            output_file_path = path.join(self.hprms.output_path, f'temporal_V{video_index:03}.pkl' )
