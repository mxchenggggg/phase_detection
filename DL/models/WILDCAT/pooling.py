...
class WildcatPool2dFunction(Function):
    ...
    def forward(ctx, input, kmax, kmin, alpha):
        ...
        kmax = WildcatPool2dFunction.get_positive_k(kmax, n)
        kmin = WildcatPool2dFunction.get_positive_k(kmin, n)
        ...
        if kmin > 0 and alpha is not 0:
            ctx.indices_min = indices.narrow(2, n - kmin, kmin)
            output.add_(sorted.narrow(
                2, n - kmin, kmin).sum(2).mul_(alpha / kmin)).div_(2)
...
class WildcatPool2d(nn.Module):
    ...
    def forward(self, input):
        return WildcatPool2dFunction.apply(
            input, self.kmax, self.kmin, self.alpha)
...
class ClassWisePoolFunction(Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = grad_output.view(
            batch_size, num_outputs, 1, h, w).expand(
