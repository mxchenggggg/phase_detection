...
class ToolTrasform:
    """ The ToolTrasform class manages data transforms for ToolDataset
    """

    def __init__(self, hparams) -> None:
        """ ToolTrasform Constructor
        """
        # Normalization
        ...
        self.train_aug = ShiftScaleRotate(shift_limit=hparams.shift_limit,
                                          ...
                                          value=hparams.value,
                                          p=hparams.p_shift)
        self.bright_aug = RandomBrightnessContrast(p=hparams.p_bright)

        # transforms for trainig, validation, testing and prediction
        self.transforms = {}
        if hparams.apply_training_aug:
            self.transforms["train"] = Compose(
                [self.resize, self.train_aug, self.bright_aug, self.normalize, ToTensorV2()])
        else:
            self.transforms["train"] = Compose(
    ...
    def add_specific_args(parser: configargparse.ArgParser):
        tool_transform_args = parser.add_argument_group(
            title='tool_transform specific args options')

        # normalization
        tool_transform_args.add_argument(
        ...
        tool_transform_args.add_argument(
            "--p_shift", type=float, default=0.7)
        tool_transform_args.add_argument(
            "--p_bright", type=float, default=0.3)
        return parser
